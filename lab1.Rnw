...
Several common \LaTeX commands are available under the Format menu, which include section, subsection, bold, italic, typewriter font, the verbatim environment and different types of lists to help you get started.

\subsection*{Equations}
For equations, you may use double \$\$ to show the begin and end of an equation that will not be numbered or
...
Regular \R start with a start ‘tag’ {\tt <<>>=} and end with an end ‘tag’ \@. They are indicated with  a different background in RStudio.  For example, the following chunk
...
You can create a new \R chunk either by manually typing the start  and end  tags, or by clicking the green “Insert a new code chunk” button on the top right of your editor window in RStudio (just to the left of the Run buttons).
...
For figures that include externally generated images, see the {\tt \\includegraphics} function.
